    ...
    if submit_button:
        ...
        for i in flattypelist:
            ...
            if pathtofile.is_file():
                ...
                plot_df['Resale_Price_Forecast'] = plot_df.apply(lambda x: x if pd.to_datetime(plot_df['Date'])>pd.to_datetime("01 March 2023") else np.nan)
                plot_df['Resale_Price'] = plot_df.apply(lambda x: x if pd.to_datetime(plot_df['Date'])<=pd.to_datetime("01 March 2023") else np.nan)
